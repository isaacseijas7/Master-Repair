...
- Cómo trabajar con Productos, Categorías, Proveedores y Órdenes
...
- **Menú lateral** (en computadora) con: Dashboard, Productos, Categorías, Proveedores y Órdenes.
- **Menú inferior** (en celular) para moverse rápido entre módulos.
- **Menú de usuario** (arriba a la derecha) para ir a Perfil o cerrar sesión.

### Cerrar sesión
1. Pulse su usuario (arriba a la derecha).
2. Seleccione **Cerrar Sesión**.

---

## 4. Dashboard (pantalla principal)
El Dashboard es un resumen general del negocio.
...
- **Ingresos Mensuales**: gráfico de barras con evolución por mes.
- **Ventas por Categoría**: gráfico circular por categoría.
- **Productos Más Vendidos**: ranking de productos.
- **Alertas de Stock Bajo**: productos por debajo del mínimo.
- **Órdenes Recientes**: últimas órdenes registradas.

### Acción útil
- Al hacer clic en varias tarjetas, el sistema lo lleva al módulo relacionado (por ejemplo, Productos u Órdenes).
...
3. Complete los datos:
   - Información básica (nombre, descripción, categoría, proveedor)
   - Precio unitario
   - Inventario (stock actual y stock mínimo)
   - Ubicación (opcional)
   - Estado activo/inactivo
4. Pulse **Crear Producto**.

### 5.3. Editar un producto
1. En la lista de productos, pulse **Editar**.
2. Cambie los datos necesarios.
3. Pulse **Guardar Cambios**.

### 5.4. Eliminar un producto
1. Pulse **Eliminar** en el producto.
...
## 8. Módulo de Órdenes
Este módulo le permite crear y controlar movimientos de inventario como ventas, compras y devoluciones.

## 8.1. Qué puede hacer
- **Nueva Orden**
- **Buscar órdenes** por número o cliente
- **Filtrar por tipo** (Venta/Compra)
...
5. Agregue productos:
   ...
   - Seleccione cantidad
   - Pulse agregar
6. Revise subtotal, impuestos, descuento y total.
7. Pulse **Crear Orden**.

## 8.4. Ver detalle de orden
...
## 8.5. Acciones desde el detalle
- **Imprimir**
- **Editar** (si está pendiente)
- **Completar** (si está pendiente)
- **Cancelar** (si está pendiente)
- **WhatsApp** (en órdenes de compra: copia un texto listo para enviar)

## 8.6. Editar una orden
- Solo se puede si está en estado **Pendiente**.
...
### 9.2. Seguridad (cambio de contraseña)
Pasos:
...
### 9.3. Información visible en el perfil
- Nombre completo
- Rol de usuario
- Email
- Estado de la cuenta
...
## 10. Búsqueda, filtros y paginación (aplica a varios módulos)
En Productos, Categorías, Proveedores y Órdenes encontrará funciones comunes:
...
## 11. Consejos rápidos para uso diario
...
## 12. Flujo recomendado de trabajo (resumen)
1. Iniciar sesión.
2. Revisar Dashboard (estado general).
3. Validar categorías y proveedores (si hay cambios).
4. Crear/actualizar productos.
5. Registrar órdenes (ventas/compras/devoluciones).
6. Gestionar órdenes pendientes hasta completarlas o cancelarlas.
7. Revisar perfil y mantener datos actualizados.
